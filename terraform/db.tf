...
resource "linode_volume" "db_volume" {
  label  = "db-volume-${var.environment}-${var.region}" # Unique label for the volume, including environment and region
  size   = 20                                           # Size in GB
...
resource "kubernetes_persistent_volume" "db_pv" {
  metadata {
    name = var.db_pv # Name of the Persistent Volume
  }
  ...
  spec {
    ...
    persistent_volume_source {
      flex_volume {
        driver = "linode/linode-blockstorage" # Linode Block Storage driver
        options = {
...
resource "kubernetes_persistent_volume_claim" "db_pvc" {
  metadata {
    name      = "${var.db_pv}-pvc" # Name of the Persistent Volume Claim
    namespace = "default"          # Namespace for the PVC
  }

  spec {
    access_modes = ["ReadWriteOnce"] # Access mode for the PVC
...
resource "kubernetes_pod" "db_pod" {
  metadata {
    name      = "db-${var.environment}" # Name of the pod
  ...
  spec {
    container {
      ...
      volume_mount {
        name       = "db-storage-${var.environment}" # Match the volume name defined below
    ...
    volume {
      name = "db-storage-${var.environment}" # Name of the volume
...
output "persistent_volume_name" {
  value       = var.db_pv # Use the variable directly for the Persistent Volume name
